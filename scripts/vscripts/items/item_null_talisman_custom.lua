...
LinkLuaModifier("modifier_item_null_talisman_custom_mana", "items/item_null_talisman_custom", LUA_MODIFIER_MOTION_NONE)
...
function modifier_item_null_talisman_custom:GetModifierBonusStats_Strength()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("str") * math.floor(GameRules:GetGameTime() / 600) end
end
function modifier_item_null_talisman_custom:GetModifierBonusStats_Agility()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("agi") * math.floor(GameRules:GetGameTime() / 600) end
end
function modifier_item_null_talisman_custom:GetModifierBonusStats_Intellect()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("int") * math.floor(GameRules:GetGameTime() / 600) end
end
function modifier_item_null_talisman_custom:GetModifierSpellAmplify_Percentage()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("spell") * math.floor(GameRules:GetGameTime() / 600) end
end


function modifier_item_null_talisman_custom:GetModifierConstantManaRegen()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("regen") * math.floor(GameRules:GetGameTime() / 600) end
end
...
self.mana = self:GetAbility():GetSpecialValueFor("mana")/self:GetAbility():GetSpecialValueFor("duration") * math.floor(GameRules:GetGameTime() / 600)
if not IsServer() then return end
