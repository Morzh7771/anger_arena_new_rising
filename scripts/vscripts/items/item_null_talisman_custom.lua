...
function modifier_item_null_talisman_custom:GetModifierBonusStats_Strength()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("str") * (GameRules:GetGameTime() / 600) end
end
function modifier_item_null_talisman_custom:GetModifierBonusStats_Agility()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("agi") * (GameRules:GetGameTime() / 600) end
end
function modifier_item_null_talisman_custom:GetModifierBonusStats_Intellect()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("int") * (GameRules:GetGameTime() / 600) end
end
function modifier_item_null_talisman_custom:GetModifierSpellAmplify_Percentage()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("spell") * (GameRules:GetGameTime() / 600) end
end


function modifier_item_null_talisman_custom:GetModifierConstantManaRegen()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("regen") * (GameRules:GetGameTime() / 600) end
end
...
self.mana = self:GetAbility():GetSpecialValueFor("mana")/self:GetAbility():GetSpecialValueFor("duration") * (GameRules:GetGameTime() / 600)
if not IsServer() then return end
