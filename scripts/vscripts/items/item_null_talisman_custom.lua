...
function modifier_item_null_talisman_custom:GetModifierSpellAmplify_Percentage()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("spell") *  (math.floor(GameRules:GetDOTATime(false,false) / 600) + 1) end
end


function modifier_item_null_talisman_custom:GetModifierConstantManaRegen()
    if self:GetAbility() then return self:GetAbility():GetSpecialValueFor("regen") *  (math.floor(GameRules:GetDOTATime(false,false) / 600) + 1) end
end
...
self.mana = self:GetAbility():GetSpecialValueFor("mana")/self:GetAbility():GetSpecialValueFor("duration") *  (math.floor(GameRules:GetDOTATime(false,false) / 600) + 1)
if not IsServer() then return end
